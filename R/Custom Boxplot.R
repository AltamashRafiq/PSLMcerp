#' Custom Box Plot Function - Made Using Highcharter Code
#'
#' @param x
#' @param var
#' @param var2
...
#'
#' @examples
custom_boxplot = function (x = NULL, var = NULL, var2 = NULL, outliers = TRUE,
                           ...)
{
  ...
  get_box_values <- function(x) {
    boxplot.stats(x)$stats %>% t() %>% cbind(boxplot.stats(x)$n) %>%
      as.data.frame() %>%
  ...
  series_box <- df %>% group_by_("g1", "g2") %>% do(data = get_box_values(.$x)) %>%
    ungroup() %>% unnest() %>% group_by_("g2") %>% do(data = list_parse(rename_(select_(.,
  ...
  series_out <- df %>% group_by_("g1", "g2") %>% do(data = get_outliers_values(.$x)) %>%
    ungroup() %>% filter(map_lgl(data, ~length(.x) != 0)) %>%
    group_by_("g2") %>% do(data = list_parse(select_(.,
